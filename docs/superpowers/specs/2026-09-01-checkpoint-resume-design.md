...
检查点的对话与控制状态只推进到已经完成且内部一致的事务。保存时机为：

1. 第一次模型调用之前；
2. 新路径第一次写入之前，先扩展写前 journal，同时保留上一完整事务的指纹；
3. 一个完整的 assistant/tool transaction 加入 `BoundedContext` 之后；
4. 固定验证器和 Reviewer 的结果已经转化为完整 transaction 之后；
5. 可恢复的停止结果返回给 CLI 之前。

同一 assistant turn 中的所有工具调用属于一个 transaction。写前保存只增加回滚记录，不推进消息或已完成状态；后续新路径的写前保存也保留上一完整事务的指纹，不能把同一批次中更早的局部写入变成新基线。只有整组工具结果齐备后，宿主才同时推进上下文、状态和当前文件指纹。这样既不会恢复出孤立的 tool message，也会在事务中途退出时因指纹不一致而拒绝误恢复。
...
初始检查点在首次模型调用前创建。每次后续保存覆盖同一个 `run_id` 文件；事务完成时它表示最近一个完整安全节点，写入开始前它还承担防止局部写入被误恢复的写前保护。模型/API 异常、进程正常中止或其他可恢复停止会保留校验通过的安全节点，并在结果中输出 `resume_available=yes`。

人工明确批准后，宿主先将检查点原子更新为 terminal 并清理敏感检查点，再提交 ChangeJournal；因此清空回滚基线时不会留下仍可恢复的活动检查点。人工拒绝或审批不可用时，先完成 rollback，再写入终态并清理。恢复入口只接受 `active`，因此即使终态文件清理失败也不能再次执行。rollback 未完成时保留 active 检查点和当前工作区供用户检查。删除失败只输出内容无关的清理告警，不能改变已经确定的业务结果。
